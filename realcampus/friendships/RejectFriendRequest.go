...
func RejectFriendRequest(db *sql.DB) fiber.Handler {
	return func(c *fiber.Ctx) error {
		utils.LogHeader("Friend Request Rejection")
		...
		// Check if the friendship exists and user is authorized to reject it.
		var exists bool
		var status string
		err := db.QueryRow(`
		...
		if !exists {
			...
			return c.Status(fiber.StatusNotFound).JSON(fiber.Map{
				"success": false,
				"error":   "Friend request not found or you're not authorized to reject it.",
			})
		}
		...
		utils.LogMessage(utils.LevelInfo, "Rejecting friend request.")
		_, err = db.Exec(`
		...
		if err != nil {
			utils.LogLineKeyValue(utils.LevelError, "Database Error", err.Error())
			utils.LogMessage(utils.LevelError, "Failed to reject friend request.")
			utils.LogFooter()
			return c.Status(fiber.StatusInternalServerError).JSON(fiber.Map{
				"success": false,
				"error":   "Database error occurred",
		...
		utils.LogMessage(utils.LevelInfo, "Friend request successfully rejected.")
		utils.LogFooter()
