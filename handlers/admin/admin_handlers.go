...
func (h *AdminHandler) GetDashboardStats(c *fiber.Ctx) error {
	...
	growthQuery := `
		SELECT 
			...
			SUM(COUNT(*)) OVER (ORDER BY DATE(creation_date)) as cumulative_count
		FROM newf 
		WHERE creation_date >= CURRENT_DATE - INTERVAL '90 days'
	...
	if err != nil {
		...
		for rows.Next() {
			...
			if err := rows.Scan(&date, &dailyRegistrations, &cumulativeCount); err == nil {
