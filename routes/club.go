...
func SetupClubRoutes(router fiber.Router, db *sql.DB, clubHandler *club.ClubHandler) {

	clubGroup := router.Group("/club", middlewares.JWTMiddleware, middlewares.NewfAuthMiddleware(db), utils.EnhanceSentryEventWithEmail)
